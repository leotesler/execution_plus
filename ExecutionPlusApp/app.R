...
mlb_pit_url <- "https://statsapi.mlb.com/api/v1/stats?stats=season&sportId=1&season=2026&group=pitching&playerPool=All&limit=20000&offset=0"
raw_mlb <- fromJSON(mlb_pit_url, simplifyVector = FALSE)
splits_mlb <- raw_mlb$stats[[1]]$splits

parse_split_mlb <- function(x) {
  ip <- as.numeric(x$stat$inningsPitched)
  if (is.null(ip) || is.na(ip) || ip == 0) return(NULL)
  tibble(
    x_mlbamid = as.integer(x$player$id),
    player_name = x$player$fullName
  )
}

pitcher_map <- map_dfr(splits_mlb, parse_split_mlb) |>
  left_join(chadwick, by = join_by(x_mlbamid == key_mlbam)) |>
  mutate(first_letter = substr(key_bbref, 1, 1))

# load aaa pitcher map ----
aaa_pit_url <- "http://statsapi.mlb.com/api/v1/stats?stats=season&sportId=11&season=2026&group=pitching&playerPool=All&limit=20000&offset=0"
...
# load st pitcher map ----
# st_url <- "https://statsapi.mlb.com/api/v1/stats?stats=season&season=2026&gameType=S&group=pitching&sportId=1&playerPool=All&limit=20000&offset=0"
# st_response <- GET(st_url)
# st_raw <- content(st_response, as = "text", encoding = "UTF-8")
# st_json <- fromJSON(st_raw, simplifyVector = FALSE)
# st_splits <- st_json$stats[[1]]$splits
# 
# parse_split_st <- function(x) {
#   ip <- as.numeric(x$stat$inningsPitched)
#   if (is.null(ip) || is.na(ip) || ip == 0) return(NULL)
#   tibble(
#     x_mlbamid = x$player$id,
#     player_name = x$player$fullName
#   )
# }
# 
# pitcher_map_st <- map_dfr(st_splits, parse_split_st) |> 
#   left_join(chadwick, by = join_by(x_mlbamid == key_mlbam)) |> 
#   mutate(first_letter = substr(key_bbref, 1, 1))
...
player_info <- function(pitcher_id, mode = c("MLB", "AAA")) {
  ...
  if (data$people$currentTeam$id < 108 | data$people$currentTeam$id > 160) {
    fa_year <- "N/A"
  } else if (nrow(map) == 0 || is.na(map$key_bbref)) {
    fa_year <- "N/A"
  } else {
  ...
  g <- grobTree(
    ...
    textGrob(paste0("2026 ", mode, " Regular Season"), y = 0.20, gp = gpar(fontsize = 16, fontface = "italic"))
  )
...
server <- function(input, output, session) {
  ...
  predictions_mlb <- reactive({
    ...
    if (!file.exists(file_path)) {
      showNotification(paste("No prediction file found for this pitcher."), type = "error")
      req(FALSE)  # stops execution cleanly
    }
