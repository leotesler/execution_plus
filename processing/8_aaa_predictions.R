...
mlb_pred <- read_csv("predictions/mlb_2025.csv")

mean_pitch_grade <- mean(mlb_pred$pitch_grade)
tidymodels_prefer()

# load milb player data ----
milb_player <- read_csv(here("data/milb_players/tong_jonah_81625.csv"))

# generate predictions ----
milb_clean <- milb_player |> 
  mutate(strikes = as.factor(strikes),
         balls = as.factor(balls))

predictions <- milb_clean |> 
  bind_cols(predict(bt_fit_final, milb_clean)) |> 
  mutate(predicted_reaa = .pred) |> 
  mutate(pitch_grade = ((percent_rank(predicted_reaa)*100)/mean_pitch_grade)*100)
...
font_add(family = "Georgia Bold", regular = "/System/Library/Fonts/Supplemental/Georgia Bold.ttf")
showtext_auto()

base_size = 12 * 1.5

custom_theme <- theme_minimal(base_family = "Georgia Bold", base_size = base_size) +
  theme(
    plot.title = element_text(family = "Georgia Bold", size = 20, face = "bold"),
    axis.title = element_text(family = "Georgia Bold", size = 16),
    axis.text = element_text(family = "Georgia Bold", size = 12)
  )

theme_set(custom_theme)

options(ggsave.dpi = 300)
...
ggplot(pitch_colors) +
  geom_tile(aes(x = 1, y = y, fill = code), width = 1, height = 1, show.legend = FALSE) +
  geom_text(
    aes(x = 0.95, y = y, label = paste0(code, ": ", name, " - ", color)),
    hjust = 1,
    size = 4,
    family = "Georgia Bold"
  ) +
...
# plot the entire visualization ----
execution_plus_card <- function(pitcher_id) {
  color_stats = c("release_speed", "pfx_z", "pfx_x", "release_spin_rate",
                  "delta_run_exp_per_100", "whiff_rate", "chase_rate",
                  "in_zone_rate", "xwoba")
...
# save predictions ----
png("execution_card.png", width = 1600, height = 1200, res = 150)
execution_plus_card(pitcher_id = as.character(predictions$pitcher[1]))
dev.off()

system("open execution_card.png")
